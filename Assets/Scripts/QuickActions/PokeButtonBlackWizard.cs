...
{
    internal class PokeButtonBlackWizard : QuickActionsWizard
    {
        private const string MENU_NAME = MENU_FOLDER +
            "Add Poke Interaction to Black Button";

        [MenuItem(MENU_NAME, priority = 200)]
        private static void OpenWizard()
        {
            ShowWindow<PokeButtonBlackWizard>(Selection.gameObjects[0]);
        }
        ...
        protected override void Create()
        {
            GameObject obj = Templates.CreateFromTemplate(
                Target.transform, Templates.PokeKeyNoteB);
